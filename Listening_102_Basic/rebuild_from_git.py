...
def rebuild_lesson(html_path, rel_path):
    # Fetch from git f7de4f7
    try:
        old_html = subprocess.check_output(['git', 'show', f'f7de4f7:{rel_path}']).decode('utf-8')
    except subprocess.CalledProcessError:
        print(f"Could not fetch {rel_path} from git, skipping.")
        return False
    ...
    for level in LEVELS:
        ...
        for lesson_folder in os.listdir(level_dir):
            if lesson_folder.startswith('Lesson_'):
                ...
                rel_path = f"{level}/{lesson_folder}/index.html"
                if os.path.exists(html_path):
                    if rebuild_lesson(html_path, rel_path):
                        count += 1
    print(f"Successfully rebuilt {count} HTML files from git commit f7de4f7!")
