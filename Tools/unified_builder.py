...
# Đường dẫn tĩnh tới thư mục gốc chứa toàn bộ các bài tập
BASE_DIR = "/Users/vungoclong/Desktop/Antigravity/UPGRADE YOUR ILETS SKILLS"

DIRS = [
    ("Reading_1232_Basic", "1232 bài luyện tập đọc hiểu"),
    ("Listening_102_Basic", "102 bài tập Nghe cơ bản"),
    ("Practice_FullTests/Listening_204_FullTest", "204 bài thi Listening Full Test"),
    ("Practice_FullTests/Reading_315_FullTest", "315 bài thi Reading Full Test")
]
...
HTML_TEMPLATE = """
    ...
    <div class="login-overlay" id="login-overlay" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.85); z-index: 99999; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(8px);">
        <div class="login-box" style="background: linear-gradient(145deg, #ffffff, #fff0f5); padding: 40px; border-radius: 30px; width: 90%; max-width: 450px; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.3); border: 4px solid #ffccd5;">
            ...
            <div class="login-subheader" style="font-size: 1rem; color: #8c6d7d; margin-bottom: 25px; font-family: 'Quicksand', sans-serif;">Hệ thống: {MODULE_TEXT} ✨</div>
            
            <div id="login-form-fields">
                <input type="text" class="login-input" id="nickname-input" placeholder="Nhập ID học viên của bạn..." autofocus 
...
JS_TEMPLATE = """
    ...
    async function grantAccess() {
        ...
        try {
            ...
            if(statusText) statusText.innerText = "Đang xác thực tài khoản qua máy chủ an toàn...";
...
def process_file(filepath, module_name):
    try:
        ...
        # 4. Inject unified overlay as first child of body
        body = soup.find('body')
        if body:
...
def main():
    ...
    for d, module_name in DIRS:
        ...
        if not os.path.exists(search_dir):
            print(f"[Cảnh báo] Không tìm thấy thư mục: {search_dir}")
            continue
        
        # Xử lý theo từng loại cấu trúc
        if d == "Reading_1232_Basic":
            # 1232 bài này dùng SPA framework nên chỉ cần tiêm vào template và index
        ...
        elif d == "Listening_102_Basic":
            # 102 bài này build tĩnh từ template.html
            ...
            # Listening_204_FullTest & Reading_315_FullTest
            # Đây là thư mục chứa hàng loạt file .html
    ...
    print(f"✅ Hoàn tất! Đã tiêm mã đăng nhập đồng nhất vào {total_processed} files.")
