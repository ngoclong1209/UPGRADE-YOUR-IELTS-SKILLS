...
def update_file(filepath, msg):
    try:
        ...
        # 1. login overlay display
        content = re.sub(r'id="login-overlay"([^>]*)display:\s*none;?', r'id="login-overlay"\1display: flex;', content)
        ...
        # 3. Disable the hiding of overlay on load
        # We find `if(overlay) overlay.style.display = "none";` and comment it out
        content = re.sub(
            r'if\s*\(\s*overlay\s*\)\s*overlay\.style\.display\s*=\s*"none"\s*;',
