...
function onEdit(e) {
  ...
  if (sheet.getName() === "Students") {
    ...
    // NẾU NHẬP TÊN HOẶC ID HỌC VIÊN => TẠO SHEET TRỐNG & CHÈN CHECKBOX
    if (col === 1 || col === 2) {
    ...
    // XỬ LÝ KHI TÍCH VÀO GÓI HỌC (CỘT E=5, F=6, G=7) THEO CẤU TRÚC HÀNG NGANG
    ...
    if (isReading || isListening || isFullTest) {
      ...
      var items = [];
      if (isFullTest) {
        // Tự động sinh ra 204 bài Listening
        for (var idx = 1; idx <= 204; idx++) {
          items.push(["", "", "Full Test Listening - Test_" + idx, "https://ngoclong1209.github.io/practicepteonline/Listening_204_FullTest/Test_" + idx + "/Test_" + idx + ".html"]);
        }
        // Tự động sinh ra 315 bài Reading
        for (var idx = 1; idx <= 315; idx++) {
          if (idx === 105) continue; // Skip 105 as requested before
          items.push(["", "", "Full Test Reading - Test_" + idx, "https://ngoclong1209.github.io/practicepteonline/Reading_315_FullTest/Test_" + idx + "/Test_" + idx + ".html"]);
        }
      } else {
      ...
      for (var i = 0; i < items.length; i++) {
        var itemName = items[i][2]; // Title
        var itemLink = items[i][3]; // URL
        if (itemName) {
    ...
    // XỬ LÝ XOÁ HỌC VIÊN BẰNG CHECKBOX (CỘT H = 8)
